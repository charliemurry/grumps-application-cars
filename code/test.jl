...
function makeCPS(;numDraws=10_000)
    ...
    df_cps = CSV.read("data/CPS_households.csv", DataFrame)
    ...
    CSV.write("temp/gmy_draws.csv",df_draws)
end
...
function makeDataForEstimation(beg_year, end_year, num_cons=nothing)
    ...
    # Consumer Sample
    df_cons = CSV.read("data/gmy_cex_consumer.csv",DataFrame)
    ...
    if num_cons !== nothing        # Further subsets the consumer data (optional)
        ...
        CSV.write("temp/consumer.csv",df_new)
    else
        CSV.write("temp/consumer.csv",df_cons)
    end
    

    # Products
    df_prod = CSV.read("data/gmy_product.csv", DataFrame)
    @subset!(df_prod, :year .>= beg_year)
    @subset!(df_prod, :year .<= end_year)
    CSV.write("temp/product.csv", df_prod)

    # Markets
    df_market = CSV.read("data/gmy_market.csv", DataFrame)
    @subset!(df_market, :year .>= beg_year)
    @subset!(df_market, :year .<= end_year)
    CSV.write("temp/market.csv", df_market)
    
    # CPS draws
    df_draws = CSV.read("temp/gmy_draws.csv", DataFrame)
    @subset!(df_draws, :year .>= beg_year)
    @subset!(df_draws, :year .<= end_year)
    CSV.write("temp/draws.csv", df_draws)   
...
function my_estimation(nodes, draws, meth)
    ...
    s = Sources(                                                            
      consumers = "temp/consumer.csv",
      products = "temp/product.csv",
      marketsizes = "temp/market.csv",
      draws = "temp/draws.csv"  
    )
    ...
    interactions =  [                                                   
        ...
        instruments = [:constant; :log_mpg; :log_hp; :log_footprint; :log_curbweight; :suv; :van; :truck; :lag_pl_con], 
...
begin
    ...
    println("Starting to make data for estimation.")
